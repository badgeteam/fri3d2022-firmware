...
static bool populate_menu(menu_t* menu) {
    ...
    return internal_result_esp32 | sdcard_result_esp32 | other_result_esp32 | internal_result_python | sdcard_result_python;
}
...
static bool show_app_details(xQueueHandle button_queue, launcher_app_t* app) {
    ...
    while (!quit) {
        ...
        if (xQueueReceive(get_input_queue(), &button_message, portMAX_DELAY) == pdTRUE) {
